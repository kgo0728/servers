...
switch(GET_ACCOUNT_SESSION_RETURN_VALUE($request['ACC_UKEY'] , $request['SESSION_KEY'] ))
{
    case 0:
    {
        ...
        if( isset($request['USE_TYPE']) && isset($request['USE_ITEM_INFO']) )
        {
        
            if(INPUT_CONTENT_LOG_ABLE > 0 )
                log_input("[RECV][ITEM_USE_EFFECT_INFO] "  . print_r($request , true) );
            ...
            $USE_ITEM_INFO    = $request['USE_ITEM_INFO']; 
            $EFFECT_IT_UKEY   = $request['EFFECT_IT_UKEY'];     //isset($request['EFFECT_IT_UKEY']) ? $request['EFFECT_IT_UKEY'] : 0;
            ...
            try 
            {
                ...
                $query = "CALL GET_RW_ITEM_USE_EFFECT_INFO(@RETURN,?,?,?,?)";
                $stmt  = $Main_Db->prepare($query);
                
                $stmt->bindParam(1, $ACC_UKEY        , PDO::PARAM_INT , 8);
                $stmt->bindParam(2, $USE_TYPE        , PDO::PARAM_INT , 2);
                $stmt->bindParam(3, $USE_ITEM_INFO   , PDO::PARAM_STR , strlen($USE_ITEM_INFO));
                $stmt->bindParam(4, $EFFECT_IT_UKEY  , PDO::PARAM_INT , 8);
                $stmt->execute();
            
                $Result_RW_Item_Use_Effect_info = array();
                while ($row = $stmt->fetch())
                {
                    $Result_RW_Item_Use_Effect_info["COIN"]                 =  $row[0];
                    $Result_RW_Item_Use_Effect_info["CRYSTAL_FR"]           =  $row[1];
                    $Result_RW_Item_Use_Effect_info["CRYSTAL_CH"]           =  $row[2];
                    $Result_RW_Item_Use_Effect_info["FIGHT_MILEAGE"]        =  $row[3];
                    $Result_RW_Item_Use_Effect_info["DEL_ITEM_UKEY"]        =  (string)$row[4];

                    /*
                    $response_server["UPDATE_TEAM_DECK"]                    =  $row[5];
                    $response_server["GET_MATCH_SERVER"]                    =  $row[6];
                    $response_server["GET_COMMUNITY_SERVER"]                =  $row[7];
                    */
                ...
                $Result_RW_User_Update_Item_Hero_Card_info = array();
                if($stmt->nextRowset() && $stmt->columnCount())
                {
                    $item_info = [];
                    while ($row = $stmt->fetch())
                    {
                        ...
                        $item_info["PROMOTION"]         =  $row[4];
                        array_push($Result_RW_User_Update_Item_Hero_Card_info ,$item_info );
                    } 
                }   
                ...
                switch(GET_QUERY_RETURN_VALUE($Main_Db, $stmt->nextRowset(),$stmt->columnCount() ))
                {
                    ...
                    case 0:
                    {
                        ...
                        $response['ITEM_USE_EFFECT_INFO'] = $Result_RW_Item_Use_Effect_info; 
                         
                        if(count($Result_RW_User_Update_Item_info) > 0 )            $response['ITEM_UPDATE_INFO']                  =  $Result_RW_User_Update_Item_info;
                        if(count($Result_RW_User_Update_Item_Hero_Card_info) > 0 )  $response['ITEM_UPDATE_HERO_CARD_INFO']        =  $Result_RW_User_Update_Item_Hero_Card_info;
