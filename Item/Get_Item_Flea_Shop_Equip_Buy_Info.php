    ...
    case 0:
    {
        ...
        if( isset($request['ACC_UKEY']) )
        {
            ...
            if(INPUT_CONTENT_LOG_ABLE > 0 )
                log_input("[RECV][ITEM_FLEA_SHOP_EQUIP_BUY_INFO] "  . print_r($request , true) );
            ...
            try 
            {
                ...
                $query = "CALL GET_RW_ITEM_FLEA_SHOP_EQUIP_BUY_INFO(@RETURN,?)";
                $stmt  = $Main_Db->prepare($query);
                
                $stmt->bindParam(1, $ACC_UKEY        , PDO::PARAM_INT , 8);
                $stmt->execute();
            
                $Result_RW_Item_Flea_Shop_Equip_Buy_info = array();
                while ($row = $stmt->fetch())
                {
                    $Result_RW_Item_Flea_Shop_Equip_Buy_info["FLEA_COIN"]              =  $row[0];
                    $Result_RW_Item_Flea_Shop_Equip_Buy_info["FLEA_BASKET_ITEM_NUM"]   =  $row[1];
                } 
                ...
                switch(GET_QUERY_RETURN_VALUE($Main_Db, $stmt->nextRowset(),$stmt->columnCount() ))
                {
                    ...
                    case 0:
                    {
                        ...
                        $response['ITEM_FLEA_SHOP_EQUIP_BUY_INFO'] = $Result_RW_Item_Flea_Shop_Equip_Buy_info; 
