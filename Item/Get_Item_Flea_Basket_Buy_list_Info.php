    ...
    case 0:
    {
        ...
        if( isset($request['ACC_UKEY']) )
        {
            ...
            if(INPUT_CONTENT_LOG_ABLE > 0 )
                log_input("[RECV][ITEM_FLEA_BASKET_BUY_LIST_INFO] "  . print_r($request , true) );
            ...
            try 
            {
                ...
                $query = "CALL GET_R_ITEM_FLEA_BASKET_BUY_LIST_INFO(@RETURN,?)";
                $stmt  = $Main_Db->prepare($query);
                
                $stmt->bindParam(1, $ACC_UKEY        , PDO::PARAM_INT , 8);
                $stmt->execute();
            
                $Result_R_Item_Flea_Basket_Buy_list_info = array();
                while ($row = $stmt->fetch())
                {
                    $item_info["ITEM_ID"]        =  $row[0];
                    $item_info["ITEM_NUM"]       =  $row[1];
                    array_push($Result_R_Item_Flea_Basket_Buy_list_info ,$item_info );
                ...
                switch(GET_QUERY_RETURN_VALUE($Main_Db, $stmt->nextRowset(),$stmt->columnCount() ))
                {
                    ...
                    case 0:
                    {
                        ...
                        if(count($Result_R_Item_Flea_Basket_Buy_list_info) > 0 ) $response['ITEM_FLEA_BASKET_BUY_LIST_INFO'] = $Result_R_Item_Flea_Basket_Buy_list_info;
