...
switch(GET_ACCOUNT_SESSION_RETURN_VALUE($request['ACC_UKEY'] , $request['SESSION_KEY'] ))
{
    case 0:
    {
        ...
        if( isset($request['SELL_IT_INFO']) )
        {
        
            if(INPUT_CONTENT_LOG_ABLE > 0 )
                log_input("[RECV][ITEM_FLEA_SHOP_EQUIP_SELL_INFO] "  . print_r($request , true) );
            ...
            $SELL_IT_INFO = $request['SELL_IT_INFO']; 
            ...
            try 
            {
                ...
                $query = "CALL GET_RW_ITEM_FLEA_SHOP_EQUIP_SELL_INFO(@RETURN,?,?)";
                $stmt  = $Main_Db->prepare($query);
                
                $stmt->bindParam(1, $ACC_UKEY        , PDO::PARAM_INT , 8);
                $stmt->bindParam(2, $SELL_IT_INFO,     PDO::PARAM_STR, strlen($SELL_IT_INFO));
                $stmt->execute();
            
                $Result_RW_Item_Flea_Shop_Equip_Sell_info = array();
                while ($row = $stmt->fetch())
                {
                    $Result_RW_Item_Flea_Shop_Equip_Sell_info["FLEA_COIN"]       =  $row[0];
                    $Result_RW_Item_Flea_Shop_Equip_Sell_info["DEL_ITEM_UKEY"]   =  $row[1];
                } 
                ...
                switch(GET_QUERY_RETURN_VALUE($Main_Db, $stmt->nextRowset(),$stmt->columnCount() ))
                {
                    ...
                    case 0:
                    {
                        ...
                        $response['ITEM_FLEA_SHOP_EQUIP_SELL_INFO'] = $Result_RW_Item_Flea_Shop_Equip_Sell_info; 
