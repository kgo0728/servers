    ...
    case 0:
    {
        ...
        if( isset($request['ACC_UKEY']) )
        {
            ...
            if(INPUT_CONTENT_LOG_ABLE > 0 )
                log_input("[RECV][ITEM_LUCK_CARD_SHOP_BUY_INFO] "  . print_r($request , true) );
            ...
            try 
            {
                ...
                $query = "CALL GET_RW_ITEM_LUCK_CARD_SHOP_BUY_INFO(@RETURN,?)";
                $stmt  = $Main_Db->prepare($query);
                
                $stmt->bindParam(1, $ACC_UKEY   , PDO::PARAM_INT , 8);
                $stmt->execute();
            
                $Result_RW_Item_Luck_Card_Buy_info = array();
                while ($row = $stmt->fetch())
                {
                    $Result_RW_Item_Luck_Card_Buy_info["COIN"]                 =  $row[0];
                    $Result_RW_Item_Luck_Card_Buy_info["CRYSTAL_FR"]           =  $row[1];
                    $Result_RW_Item_Luck_Card_Buy_info["CRYSTAL_CH"]           =  $row[2];
                    $Result_RW_Item_Luck_Card_Buy_info["LUCK_CARD_BUY_NUM"]    =  $row[3];
                ...
                switch(GET_QUERY_RETURN_VALUE($Main_Db, $stmt->nextRowset(),$stmt->columnCount() ))
                {
                    ...
                    case 0:
                    {
                        ...
                        $response['ITEM_LUCK_CARD_SHOP_BUY_INFO'] = $Result_RW_Item_Luck_Card_Buy_info; 
