    ...
    case 0:
    {
        ...
        if( isset($request['ACC_UKEY']) )
        {
            ...
            if(INPUT_CONTENT_LOG_ABLE > 0 )
                log_input("[RECV][ITEM_UPDATE_HERO_CARD_INFO] "  . print_r($request , true) );
            ...
            try 
            {
                ...
                $query = "CALL GET_R_ITEM_UPDATE_HERO_CARD_INFO(@RETURN,?)";
                $stmt  = $Main_Db->prepare($query);
                
                $stmt->bindParam(1, $ACC_UKEY        , PDO::PARAM_INT , 8);
                $stmt->execute();
            
                $Result_R_User_Update_Item_Hero_Card_info = array();
                while ($row = $stmt->fetch())
                {
                    $item_info["IT_UKEY"]           =  (string)$row[0];
                    $item_info["CONDITION"]         =  $row[1];
                    array_push($Result_R_User_Update_Item_Hero_Card_info ,$item_info );
                } 
                ...
                switch(GET_QUERY_RETURN_VALUE($Main_Db, $stmt->nextRowset(),$stmt->columnCount() ))
                {
                    ...
                    case 0:
                    {
                        ...
                        if(count($Result_R_User_Update_Item_Hero_Card_info) > 0 )   $response['ITEM_UPDATE_HERO_CARD_INFO']  =  $Result_R_User_Update_Item_Hero_Card_info;
