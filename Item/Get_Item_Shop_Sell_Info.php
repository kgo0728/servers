...
switch(GET_ACCOUNT_SESSION_RETURN_VALUE($request['ACC_UKEY'] , $request['SESSION_KEY'] ))
{
    case 0:
    {
        ...
        if( isset($request['IT_UKEY']) && isset($request['USE_NUM']) )
        {
        
            if(INPUT_CONTENT_LOG_ABLE > 0 )
                log_input("[RECV][ITEM_SHOP_SELL_INFO] "  . print_r($request , true) );
            ...
            try 
            {
                ...
                $query = "CALL GET_RW_ITEM_SHOP_SELL_INFO(@RETURN,?,?,?)";
                $stmt  = $Main_Db->prepare($query);
                
                $stmt->bindParam(1, $ACC_UKEY        , PDO::PARAM_INT , 8);
                $stmt->bindParam(2, $IT_UKEY         , PDO::PARAM_INT , 8);
                $stmt->bindParam(3, $USE_NUM         , PDO::PARAM_INT , 4);
                $stmt->execute();
            
                $Result_RW_Item_Shop_Sell_info = array();
                while ($row = $stmt->fetch())
                {
                    $Result_RW_Item_Shop_Sell_info["COIN"]            =  $row[0];
                    $Result_RW_Item_Shop_Sell_info["DEL_ITEM_UKEY"]   =  (string)$row[1];
                } 
                ...
                switch(GET_QUERY_RETURN_VALUE($Main_Db, $stmt->nextRowset(),$stmt->columnCount() ))
                {
                    ...
                    case 0:
                    {
                        ...
                        $response['ITEM_SHOP_SELL_INFO'] = $Result_RW_Item_Shop_Sell_info; 
