...
switch(GET_ACCOUNT_SESSION_RETURN_VALUE($request['ACC_UKEY'] , $request['SESSION_KEY'] ))
{
    case 0:
    {
        ...
        if( isset($request['IT_UKEY']) )
        {
        
            if(INPUT_CONTENT_LOG_ABLE > 0 )
                log_input("[RECV][ITEM_UPGRADE_HERO_CARD_INFO] "  . print_r($request , true) );
            ...
            try 
            {
                ...
                $query = "CALL GET_RW_ITEM_UPGRADE_HERO_CARD_INFO(@RETURN,?,?)";
                ...
                $Result_RW_Item_Upgrade_Hero_Card_info = array();
                while ($row = $stmt->fetch())
                {
                    $Result_RW_Item_Upgrade_Hero_Card_info["COIN"]                 =  $row[0];
                    $Result_RW_Item_Upgrade_Hero_Card_info["CRYSTAL_FR"]           =  $row[1];
                    $Result_RW_Item_Upgrade_Hero_Card_info["CRYSTAL_CH"]           =  $row[2];

                    $Result_RW_Item_Upgrade_Hero_Card_info["DEL_ITEM_UKEY_INFO"]   =  $row[3];

                    $Result_RW_Item_Upgrade_Hero_Card_info["IT_UKEY"]              =  (string)$row[4];
                    $Result_RW_Item_Upgrade_Hero_Card_info["GRADE"]                =  $row[5];
                    $Result_RW_Item_Upgrade_Hero_Card_info["PROMOTION"]            =  $row[6];
                ...
                switch(GET_QUERY_RETURN_VALUE($Main_Db, $stmt->nextRowset(),$stmt->columnCount() ))
                {
                    ...
                    case 0:
                    {
                        ...
                        $response['ITEM_UPGRADE_HERO_CARD_INFO'] = $Result_RW_Item_Upgrade_Hero_Card_info; 
