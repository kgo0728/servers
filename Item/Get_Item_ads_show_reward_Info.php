...
<?php
    ...
    if( isset($request['ACC_UKEY']) && isset($request['UU_DB_PATH']) && isset($request['TYPE']) )
    {
        ...
        try 
        {
            ...
            $query = "CALL GET_R_ITEM_ADS_SHOW_REWARD_INFO(@RETURN,?)";
            ...
            $Result_R_Item_Ads_show_Result_info = '0';
            while ($row = $stmt->fetch())
            {
                $Result_R_Item_Ads_show_Result_info   =  $row[0];
            ...
            switch(GET_QUERY_RETURN_VALUE($Main_Db, $stmt->nextRowset(),$stmt->columnCount() ))
            {
                ...
                case 0:
                {
                    ...
                    $query = "CALL GET_RW_ITEM_ADS_SHOW_REWARD_INFO(@RETURN,?,?)";
                    $stmt  = $User_Db->prepare($query);
        
                    $stmt->bindParam(1, $ACC_UKEY  , PDO::PARAM_INT , 8);
                    $stmt->bindParam(2, $Result_R_Item_Ads_show_Result_info , PDO::PARAM_STR , strlen($Result_R_Item_Ads_show_Result_info));
                    $stmt->execute();      

                  
                    $Result_User_Item_Ads_Reward_info = array();
                    while ($row = $stmt->fetch())
                    {
                          $Result_User_Item_Ads_Reward_info["TICKET"]     =  $row[0];
                    ...
                    switch(GET_QUERY_RETURN_VALUE($User_Db, $stmt->nextRowset(),$stmt->columnCount() ))
                    {
                        ...
                        case 0:
                        {
                            ...
                            $response['ADS_SHOW_REWARD_INFO']["TICKET"] = $Result_User_Item_Ads_Reward_info["TICKET"] ;
