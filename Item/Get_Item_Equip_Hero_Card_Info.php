    ...
    case 0:
    {
        ...
        if( isset($request['HERO_IT_UKEY']) && isset($request['EQUIP_IT_UKEY']) )
        {
            ...
            if(INPUT_CONTENT_LOG_ABLE > 0 )
                log_input("[RECV][ITEM_EQUIP_HERO_CARD_INFO] "  . print_r($request , true) );
            ...
            try 
            {
                ...
                $query = "CALL GET_RW_ITEM_EQUIP_HERO_CARD_INFO(@RETURN,?,?,?)";
                ...
                $Result_RW_Item_Equip_Hero_Card_info = array();
                while ($row = $stmt->fetch())
                {
                    $Result_RW_Item_Equip_Hero_Card_info["IT_UKEY"]           =  (string)$row[0];
                    $Result_RW_Item_Equip_Hero_Card_info["EQUIP_CARD_UKEY"]   =  (string)$row[1];
                } 
                ...
                switch(GET_QUERY_RETURN_VALUE($Main_Db, $stmt->nextRowset(),$stmt->columnCount() ))
                {
                    ...
                    case 0:
                    {
                        ...
                        $response['ITEM_EQUIP_HERO_CARD_INFO'] = $Result_RW_Item_Equip_Hero_Card_info; 
