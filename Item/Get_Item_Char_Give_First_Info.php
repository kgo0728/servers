...
<?php
    ...
    switch(GET_ACCOUNT_SESSION_RETURN_VALUE($request['ACC_UKEY'] , $request['SESSION_KEY'] ))
    {
        case 0:
        {
            ...
            if( isset($request['GIVE_TYPE']) &&  isset($request['SELECT_ID']) )
            {
                if(INPUT_CONTENT_LOG_ABLE > 0 )
                    log_input("[RECV][ITEM_CHAR_GIVE_FIRST_INFO] "  . print_r($request , true) );
                ...
                try 
                {
                    ...
                    $query = "CALL GET_RW_ITEM_CHAR_GIVE_FIRST_INFO(@RETURN,?,?,?)";
                    $stmt  = $Main_Db->prepare($query);
                    
                    $stmt->bindParam(1, $ACC_UKEY   , PDO::PARAM_INT , 8);
                    $stmt->bindParam(2, $GIVE_TYPE  , PDO::PARAM_INT , 2);
                    $stmt->bindParam(3, $SELECT_ID  , PDO::PARAM_INT , 2);
                    $stmt->execute();
                    ...
                    switch(GET_QUERY_RETURN_VALUE($Main_Db, $stmt->nextRowset(),$stmt->columnCount() ))
                    {
                        ...
                        case 0:
                        {
                            ...
                            $response['CHAR_GIVE_FIRST_INFO']['GIVE_TYPE'] = $GIVE_TYPE;
                            $response['CHAR_GIVE_FIRST_INFO']['SELECT_ID'] = $SELECT_ID;
